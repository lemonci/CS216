...
int preval(FILE * f) {
	int token = gettoken (f); // get first token
	if (token >= 0) return token; // it is a number, return the number
	int a = preval(f); // evaluate the first operant
	int b = preval(f); // evaluate the first operant
	switch (-token) // do the calculation
		{
			case '+': return a + b;
			case '-': return a - b;
			case '*': return a * b;
